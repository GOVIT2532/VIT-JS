...
const myBot = ['u6c8aab6ee167a596be2cf045ee2f90df','udba7a6d8efcd6d0a4dfe0a7290f15b68'];
...
\n\n# http://line.me/ti/p/4bvwOIMft8  ^_^";
    ...
    poll(operation) {
		...
		if(operation.type == 17 && this.stateStatus.salam == 1 && isAdminOrBot(operation.param2)) {//ada yang join
			...
			halobos.toType = 2;
			halobos.text = "[By.☬ധู้さန້ণق↔ധഖาໄฟ☬] !";
...
\n";
        ...
        if(txt === 'ออกไป' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
